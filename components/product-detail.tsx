...
import { ChevronDown, ChevronUp, Search, Shield, Thermometer, Wind, RotateCcw, Shirt, Droplets, ArrowLeft } from "lucide-react"
...
function ProductDetailContent({ slug }: ProductDetailProps) {
  ...
  const product = products.find((p) => p.slug === slug) || {
    ...
    images: Array.from({ length: 8 }, (_, i) => `/placeholder.svg?height=600&width=400&text=Vista+${i + 1}`),
    colors: [
  ...
  const [selectedColor, setSelectedColor] = useState(product.colors?.[0]?.color || "beige")
  ...
  const getCurrentColorImages = () => {
    if (!product.colorImageAssociations || product.colorImageAssociations.length === 0) {
  ...
  const handleImageChange = (imageIndex: number) => {
    ...
    if (selectedImageData) {
      ...
      const colorImageIndex = colorImages.findIndex(img => img === selectedImageData.url)
  ...
  const getRelatedProducts = () => {
    ...
    const sameGenderProducts = allProducts.filter(p => p.gender === product.gender)
    ...
    const complementaries = complementaryCategories[currentCategory as keyof typeof complementaryCategories] || []
    ...
    const otherSameGenderProducts = sameGenderProducts.filter(p => 
      !complementaries.includes(p.category?.toLowerCase() || '')
    )
    
    // Combinar y tomar máximo 4 productos
    const allRelatedProducts = [...complementaryProducts, ...otherSameGenderProducts]
  ...
  const careInstructions = [
    { icon: <Thermometer className="h-4 w-4" />, text: "LAVAR A MÁQUINA MAX 30°C CENTRIFUGADO CORTO" },
    ...
    <>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-sm text-gray-500 dark:text-gray-400 mb-6">
          ...
          |{" "}
          <Link href={`/${product.gender}?category=${product.category}`} className="hover-yellow">
            {product.category}
          </Link>{" "}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-6 lg:space-y-0 lg:flex lg:gap-4">
            ...
            <div className="hidden lg:block relative w-24">
              <div
                className="flex flex-col gap-2 max-h-[600px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent"
              >
                {allImagesWithColors.map((imageData, index) => (
                  <div key={`${imageData.color}-${imageData.url}-${index}`} className="relative">
                    <button
                    ...
                    <div 
                      ...
                      style={{ 
                        backgroundColor: imageData.color === "blanco" 
                          ? "#FFFFFF" 
                          : product.colors?.find(c => c.color === imageData.color)?.hex || '#808080' 
                      }}
            ...
            <div className="w-full lg:flex-1 relative">
              <div className="relative bg-gray-100 dark:bg-gray-800">
                <img
                  src={currentColorImages[selectedImage] || product.image || "/placeholder.svg"}
                  alt={`${product.name} - ${product.colors?.find(c => c.color === selectedColor)?.name || 'Default'}`}
            ...
            <div className="lg:hidden">
              <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
                {allImagesWithColors.map((imageData, index) => (
                  <div key={`mobile-${imageData.color}-${imageData.url}-${index}`} className="relative flex-shrink-0">
                    <button
                      onClick={() => handleImageChange(index)}
          ...
          <div className="space-y-6">
            <div>
              <h1 className="typography-primary text-3xl font-bold mb-2 dark:text-white">{product.name}</h1>
              <div className="h-1 w-12 bg-black dark:bg-white mb-4"></div>
              <div className="space-y-2">
                <div className="flex items-baseline gap-4">
                  <p className="typography-accent text-3xl font-bold dark:text-white">${product.price?.toLocaleString() || '0'}</p>
                  <span className="text-sm text-gray-600 dark:text-gray-300">Hasta 3 cuotas sin interés</span>
                </div>
                <div className="flex items-center gap-3 bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-lg border border-yellow-200 dark:border-yellow-800">
                  <div className="flex-1">
                    <p className="text-lg font-bold text-yellow-800 dark:text-yellow-200">
                      DESCUENTO EFECTIVO: <span className="text-xl text-yellow-900 dark:text-yellow-100">${product.cashPrice?.toLocaleString() || Math.floor(product.price * 0.75).toLocaleString()}</span>
                    </p>
                  </div>
                  <span className="text-lg text-yellow-900 dark:text-yellow-100 font-bold bg-yellow-200 dark:bg-yellow-800 px-3 py-1 rounded-full">25% OFF</span>
                </div>
              </div>
            </div>
            ...
            {product.colors && product.colors.length > 0 && (
              <div>
                <h3 className="typography-secondary font-medium mb-3 dark:text-white">
                  Color: <span className="text-gray-600 dark:text-gray-400">{product.colors.find(c => c.color === selectedColor)?.name || 'Seleccionar'}</span>
                </h3>
            ...
            {product.sizes && product.sizes.length > 0 && (
              <div>
                <h3 className="typography-secondary font-medium mb-3 dark:text-white">
                  Talle: <span className="text-gray-600 dark:text-gray-400">{selectedSize || 'Seleccionar'}</span>
                </h3>
                ...
                {expandedSections.includes("code") && (
                  <div className="pb-4 space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="dark:text-white">{product.productCode || product.code || "Sin código"}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600 dark:text-gray-400">Categoría</span>
                      <span className="dark:text-white capitalize">{product.category || "Sin categoría"}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600 dark:text-gray-400">Género</span>
                      <span className="dark:text-white capitalize">{product.gender || "Unisex"}</span>
                    </div>
                  </div>
                )}
              </div>
                  ...
                  {expandedSections.includes("sizes") && (
                    <div className="pb-4">
                      <div className="bg-white dark:bg-gray-800 p-4">
                        <h4 className="font-bold mb-4 dark:text-white">CÓMO MEDIRTE</h4>
                        <div className="flex gap-6">
                          ...
                          <div className="flex-2 space-y-3">
                            <div>
                              <span className="font-medium text-black dark:text-white">1. Contorno cintura:</span>
                              <span className="text-gray-700 dark:text-gray-300">
                                {" "}
                                Coloca la cinta métrica rodeando la parte más estrecha del contorno de tu cintura.
                              </span>
                            </div>
                            <div>
                              <span className="font-medium text-black dark:text-white">2. Contorno cadera:</span>
                              <span className="text-gray-700 dark:text-gray-300">
                                {" "}
                                Junta los pies y coloca la cinta métrica rodeando la parte más ancha del contorno de tu
                                cadera.
                              </span>
                            </div>
                          </div>
                ...
                {expandedSections.includes("care") && (
                  <div className="pb-0">
                    <p className="text-gray-600 dark:text-gray-300 mb-4">
                      Cuidar de tus prendas es una manera de alargar su vida útil. Sigue las instrucciones de cuidado y
                      lava tus prendas solo cuando sea necesario. Reduciendo los lavados y los secados alargamos la vida
                      de nuestras prendas y reducimos el consumo de agua y energía.
                    </p>
                    <div className="space-y-3">
                      {careInstructions.map((instruction, index) => (
                        <div key={index} className="flex items-center gap-3">
                          <div className="text-black dark:text-white">{instruction.icon}</div>
                          <span className="text-sm dark:text-gray-300">{instruction.text}</span>
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        {relatedProducts.length > 0 && (
          <div className="mt-16">
            <h2 className="typography-secondary text-2xl font-bold mb-8 dark:text-white">COMPLETÁ TU LOOK</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {relatedProducts.map((relatedProduct, index) => (
                <Link href={`/producto/${relatedProduct.slug}`} key={`${relatedProduct.id}-${index}`} className="text-center group">
